...
export function BlockedUsersDialog({
  ...
  const loadBlockedUsers = useCallback(async () => {
    ...
    if (loadError) {
      setError(publicErrorMessage(loadError, "Não foi possível abrir as pessoas bloqueadas."));
    } else {
  ...
  async function unblock(user: BlockedUser) {
    ...
    if (unblockError)
      return toast.error(publicErrorMessage(unblockError, "Não foi possível desbloquear agora."));
              ...
              {users.map((user) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="inline-flex shrink-0 items-center gap-1.5 rounded-full border-2 border-foreground bg-background px-3 py-2 text-xs font-black shadow-[2px_2px_0_var(--foreground)] disabled:opacity-50"
                  >
